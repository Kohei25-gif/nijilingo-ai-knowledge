...
1. [検証で判明した重要事実](#1-検証で判明した重要事実)
2. [現状分析：理想と現実](#2-現状分析理想と現実)
3. [日本語→英語翻訳の核心課題](#3-日本語英語翻訳の核心課題)
4. [研究ベースのプロンプト設計](#4-研究ベースのプロンプト設計)
5. [コード分析と実装ガイド](#5-コード分析と実装ガイド)
6. [詳細実装ガイド](#6-詳細実装ガイド)
7. [検証と改善サイクル](#7-検証と改善サイクル)
...
**⚠️ これらは別問題！混同すると原因特定がズレる**

### 1.3 日英乖離の詳細

```
【理想的な状態】
スライダー 0%  → 英語: "Hello" / 日本語: "こんにちは"
スライダー 50% → 英語: "Hey there!" / 日本語: "やあ！"
スライダー 100% → 英語: "Yo!" / 日本語: "よっ！"
         ↓
    英語と日本語が対応している ✅

【現在の問題】
スライダー 0%  → 英語: "Hello" / 日本語: "こんにちは"
スライダー 50% → 英語: "Hello" / 日本語: "やあ！"  ← 英語が変わってない！
スライダー 100% → 英語: "Hello" / 日本語: "よっ！" ← 英語が変わってない！
         ↓
    日本語は変わるが英語が対応していない ❌
```

**核心問題**: 翻訳（英語）と逆翻訳（日本語）が別々のタイミングで生成され、
キャッシュのキーがずれている可能性
...
│    → キャッシュ更新が間に合わない                       │
│                                                         │
│ 🔴 最重要: 日英乖離（日本語変わるが英語変わらない）     │
│    → 翻訳と逆翻訳の生成タイミングがずれている           │
│                                                         │
│ 🟡 待つと変わる                                         │
│    → バックグラウンド生成完了後にUIが更新される         │
│                                                         │
│ 🔴 名詞認識エラー（「おうた」→「singing」）              │
│    → 固有名詞と一般名詞の区別ができていない             │
│                                                         │
...
| 問題 | 根本原因 | 影響範囲 | 優先度 |
|------|----------|----------|--------|
| 日英乖離 | 翻訳と逆翻訳が別エージェントで生成、キャッシュキーずれ | データ品質 | 🔴最重要 |
| 操作しないと変わらない | 事前生成完了前にスライダー操作 | UI/UX | 🔴最重要 |
| 待つと変わる | バックグラウンド生成完了後にUI更新 | UI/UX | 🟡中 |
| 名詞誤認識 | 文脈解析なしの単純な翻訳 | 正確性 | 🟡中 |
| 主語省略誤解 | ゼロ照応解決ロジックなし | 正確性 | 🟡中 |
| modality変更 | PARTIAL編集の制約が緩い | 品質 | 🟢低 |
...
主要ファイル:
📱 App（メインアプリ）:
https://github.com/Kohei25-gif/nijilingo-ai-knowledge/blob/main/src/App_日本語ベース.tsx

🌐 翻訳API（プロンプト含む）:
https://github.com/Kohei25-gif/nijilingo-ai-knowledge/blob/main/src/groq_日本語ベース.ts
...
### 5.3 問題3: ロード速度問題（100ms遅延）

```typescript
// App.tsx - 1550行目付近の問題
// 現状: handleToneSelectで100ms遅延あり

const handleToneSelect = (toneId: string) => {
  // ...
  setTimeout(async () => {
    for (const tone of otherTones) {
      await fetchAllBucketsForTone(tone, ...)
    }
  }, 100)  // ← この100msを0msにしたい
}
```
...
const handleToneSelect = (toneId: string) => {
  ...
  // バックグラウンド処理は非同期で即座に開始（0ms遅延）
  Promise.resolve().then(async () => {
    for (const tone of otherTones) {
      // 選択中のトーンが変わっていたら中止
      if (selectedTone !== toneId) {
        console.log('[Background] Aborted: tone changed')
        return
      }
      await fetchAllBucketsForTone(tone, ...)
    }
...
**ただし注意点**：
- 0msにするとUIスレッドをブロックする可能性
- 実際には `requestIdleCallback` または `setTimeout(0)` を使用
...
const handleToneSelect = (toneId: string) => {
  ...
  // requestIdleCallbackを使用（ブラウザが暇な時に実行）
  if ('requestIdleCallback' in window) {
    requestIdleCallback(async () => {
      for (const tone of otherTones) {
        if (selectedTone !== toneId) return
        await fetchAllBucketsForTone(tone, ...)
      }
    }, { timeout: 1000 })
  } else {
